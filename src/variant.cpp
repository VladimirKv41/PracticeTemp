...
Variant& Variant::operator= (const Variant& a_variant)
{
	...
	if(m_var_type == var_type::INT64)
		m_pointer_to_value = new int64_t(a_variant.getvalue<int64_t>());
	else if (m_var_type == var_type::UINT64)
		m_pointer_to_value = new uint64_t(a_variant.getvalue<uint64_t>());
	else if (m_var_type == var_type::BOOL)
		m_pointer_to_value = new bool(a_variant.getvalue<bool>());
	else if (m_var_type == var_type::LDOUBLE)
		m_pointer_to_value = new long double(a_variant.getvalue<long double>());
	else if (m_var_type == var_type::CHAR)
		m_pointer_to_value = new char(a_variant.getvalue<char>());
	else if (m_var_type == var_type::STRING)
		m_pointer_to_value = new std::string(a_variant.getvalue<std::string>());
	else if (m_var_type == var_type::WSTRING)
		m_pointer_to_value = new std::wstring(a_variant.getvalue<std::wstring>());
	else if (m_var_type == var_type::TIME)
		m_pointer_to_value = new Time(a_variant.getvalue<Time>());
	else if (m_var_type == var_type::DATE)
		m_pointer_to_value = new Date(a_variant.getvalue<Date>());
	else if (m_var_type == var_type::DATETIME)
		m_pointer_to_value = new DateTime(a_variant.getvalue<DateTime>());
	return *this;
}
