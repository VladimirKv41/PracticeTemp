 ...
 */
make_result Selection::make(const std::string& a_dim_name, const std::vector<std::string>& a_marks_list, const std::set<std::string>& a_measure_list) 
{
	...
	if (m_selection_points.empty()) {
		making((*it_dim)->get_marks(), a_dim_name, a_marks_list, a_measure_list);
	}
	else {
		making(m_selection_points, a_dim_name, a_marks_list, a_measure_list);
	}
	// Найдены ли факты
	if (m_selection_points.empty())
...
)
{
	for (auto it_dim_mark = a_marks_list.begin(); it_dim_mark != a_marks_list.end(); it_dim_mark++) {
		...
		get_DataPoints_by_measure(m_selection_points[{a_dim_name, * it_dim_mark}], a_map_from.at(*it_dim_mark)->get_DataPoints(), a_measure_list);
...
)
{
	...
	for (auto it_dim_mark = a_marks_list.begin(); it_dim_mark != a_marks_list.end(); it_dim_mark++) {
		...
		if (a_map_from.find({ a_dim_name, *it_dim_mark }) == a_map_from.end())
			continue;
		// Добавление точек данных
		get_DataPoints_by_measure(temp_selection_points[{a_dim_name, * it_dim_mark}], a_map_from.at({ a_dim_name, *it_dim_mark }), a_measure_list);
		// Была ли найдена хоть одна точка данных
...
void Selection::add_rest_fact_points(dpoint_ummaps_map& a_map, const std::string& a_dim_name, const std::string& a_dim_mark) {
	auto& aab = a_map[{a_dim_name, a_dim_mark}];
	for (auto it_dpoint = aab.begin(); it_dpoint != aab.end(); it_dpoint++) {
		...
		for (auto it_dpoint2 = fact_datapoints.begin(); it_dpoint2 != fact_datapoints.end(); it_dpoint2++) {
			...
			if (dim_name != a_dim_name) {
				a_map[{dim_name, (*it_dpoint2)->get_dim_mark_name()}].emplace((*it_dpoint).second->get_Fact()->get_Measure_name() ,*it_dpoint2);
			}
		}
	}
}
